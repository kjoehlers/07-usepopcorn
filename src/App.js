...
function Search({ query, setQuery }) {
  ...
  function handleChange(e) {
    ...
    // set a new timer to update the query after a delay (debouncing)
    const newTimer = setTimeout(() => {
      setQuery(newSearchString.trim());
    }, 500);
  ...
  // Cleanup: This useEffect is used to clear the timer when the component unmounts
  useEffect(() => {
    return () => {
      if (timer) {
        clearTimeout(timer);
